package co.grupo1.proyecto.thymeleaf.config;
...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // Ensure each demo page exists — if the repo already has some entries
        // (e.g. from a previous run or DB), we only insert missing ones.
        List<wikipage> seed = List.of(
                new wikipage(
        ...
        for (wikipage p : seed) {
            // if slug missing in DB, insert it
            if (wikiPageRepository.findBySlug(p.getSlug()).isEmpty()) {
                wikiPageRepository.save(p);
            }
        }
    }
}